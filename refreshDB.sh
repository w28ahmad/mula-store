...
if [[ $1 == "--prod" ]]; then
    if [[ -z "${RDS_HOST}" ]]; then
        ...
        mysql -h ${RDS_HOST} --password=${RDS_PASSWORD} --user=${DB_USER} < createQuestionStore.sql 2>&1
        
        echo '[LOG] Migrating Data'
        mysql -h ${RDS_HOST} --password=${RDS_PASSWORD} --user=${DB_USER} < insertQuestionStore.sql 2>&1
    fi
else
    if [[ -z "${DB_HOST}" ]]; then
